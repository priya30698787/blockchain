...
contract Voting_1 {
    ...
    constructor(bytes32[] memory proposalNames, address[] memory voter_list) {
        ...
        stop= start+100;        //endTime is set to 100 seconds after the voting process starts
 ...
 struct Candidate {
    ...
    function winnerName() external view returns (string memory) {
     ...
     function IndianParty(int Alice , int Betty , int Cecilia ) pure public returns (int) {
        int Mysum = Alice + Betty + Cecilia;
        return Mysum;
     }
     function IndianParty() public view returns (uint AliceVotes, uint BettyVotes, uint CeciliaVotes) {
    for (uint i = 0; i < proposals.length; i++) {
        if (proposals[i].name == "Alice") {
            AliceVotes = proposals[i].voteCount;
        } else if (proposals[i].name == "Betty") {
            BettyVotes = proposals[i].voteCount;
        } else if (proposals[i].name == "Cecilia") {
            CeciliaVotes = proposals[i].voteCount;
        }
    }
